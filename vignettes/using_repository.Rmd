...
For a fuller treatment of this material, please read this **drat** [vignette for package authors](https://cran.r-project.org/web/packages/drat/vignettes/DratForPackageAuthors.html). The original purpose of **drat** was to personal repositories publically available on `github.com`. 
...
The reason for you, as the adminstrator, to set the `dratRepo` option is to make it easier for you to add a package to your repository; **drat** will know where to look.
...
Let's say you have a package called **boomerang**, and it ready to add to the ACMERAN repository. We will assume that the package-project for **boomerang** is on your computer (having cloned it, if needed), and that your `option("dratRepo")` is set properly. Start by opening the **boomerang** project in your RStudio IDE, ensuring your working directory is the project's root direcory.
...
One note, in my experience it may occasionally be necessary to install packages using the `type` option, `install.packages("boomerang", type = "source")`. As **drat** explains, its purpose is to act only as a source repository, as opposed to a binary repository. As a result, if you add pacakages to ACMERAN that require compilation, your users will need the appropriate compilation-environment, e.g. [Rtools](https://cran.r-project.org/bin/windows/Rtools/) for Windows.
...
From inside the firewall, you could pull the ACMERAN repository from your GHE, then push it to (private) `github.com`. From your resource outside the firewall, you could then pull the repository from `github.com` to a directory on that resource. Also on that resource, you could amend an `.Rprofile` file (or `.Rprofile.site`). Keep in mind that the URL that you provide for a package repository can be a `file://` URL on your local machine.

```{r}

```

Given the rise of Docker, it occurs to me that this may be a use-case for it. However, I have to confess to not having used Docker - so I am in no position to recommend it (or not).
