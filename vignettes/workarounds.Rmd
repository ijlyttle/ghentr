...
In the initial shakedown of this package, my colleage Emily Bosak and I found some glitches.

## Too many repos

### Glitch

This has already been brought to the [attention of the devtools team](https://github.com/hadley/devtools/issues/1695), but it seems that if your collection of repositories is too long, `devtools::build()` runs into a problem. The error looks like this:
...
The function `git2r::cred_ssh_key()` looks for your keys in the diectory `"~/.ssh"`. On Windows, this may be problematic. When we type `path.expand("~")`, we get `"C:/Users/username/Documents"`. This means that `git2r::cred_ssh_key()` looks for our keys in `"C:/Users/username/Documents/.ssh"`; they aren't there!

### Workaround

We solved this problem by creating a symbolic directoy-link so that we need keep only one set of keys. Following the Windows "Start" button, we opened the command line, using "Run as administrator".
